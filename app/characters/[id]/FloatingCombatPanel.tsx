...
export default function FloatingCombatPanel({ name, level, hp, ac, speed, proficiencyBonus, mods, saveMod }: Props) {
  ...
  useEffect(() => {
    ...
    if (!saved?.pos) {
      setPos({ x: window.innerWidth - 230, y: window.innerHeight - 340 });
    }
  }, []);
  ...
  useEffect(() => {
    if (pos === null) return;
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ visible, collapsed, pos }));
    } catch {}
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      setPos({
        ...
        y: Math.max(0, Math.min(e.clientY - dragOffset.current.y, window.innerHeight - 60 - panelH)),
      });
    };
  ...
  // Don't render until position is known (avoids SSR mismatch)
  ...
  return (
    ...
    >
      ...
      {collapsed ? (
        <div style={{ padding: '5px 10px', display: 'flex', gap: 8, alignItems: 'center', fontFamily: 'var(--font-display)', fontSize: 11 }}>
          ...
          <span style={{ color: 'var(--border-light)' }}>|</span>
          <span style={{ color: 'var(--ink)', fontWeight: 700 }}>INI {fmt(mods.DEX)}</span>
        </div>
      ) : (
        <div style={{ padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 4 }}>
            {([
              ...
              <div key={label} style={{ textAlign: 'center', background: 'var(--parchment-dark,#ede8df)', border: '1px solid var(--border-light)', borderRadius: 3, padding: '4px 2px' }}>
